...
Status: BLOCKED
Summary:
- 需要真实 Outlook OAuth2 凭据才能继续验证
- 单元测试已通过（test_mail_fetchers.py 7 tests, test_mail_api.py 5 tests），但都使用 mock
...
Notes:
阻塞原因：
- 没有配置 OUTLOOK_CLIENT_ID / OUTLOOK_REFRESH_TOKEN 或等效环境变量
- .env 文件中没有找到 Outlook/Azure 相关配置
- 需要用户提供：
  1. Outlook OAuth2 client_id
  2. 一个有效的 refresh_token（可以是测试账号的）
  3. 测试目标邮箱地址
解除阻塞后，按任务文件的 checklist 依次验证 Graph API token 刷新和真实取件。
